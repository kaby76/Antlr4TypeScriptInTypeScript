...
files2=`dotnet trglob -- '../examples/*.ts' | grep -v '.errors$' | grep -v '.tree$'`
files=()
for f in $files2
do
...
# rm -rf `find ../examples/*.ts -type f -name '*.errors' -o -name '*.tree' -size 0`
...
then
    gen=`find ../examples/*.ts -type f -name '*.errors' -o -name '*.tree'`
    if [ "$gen" != "" ]
    then
        dos2unix -f $gen
    fi
